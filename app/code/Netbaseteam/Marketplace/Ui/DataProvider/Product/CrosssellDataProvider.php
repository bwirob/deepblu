...
class CrosssellDataProvider extends ProductDataProvider
{
    /**
    ...
    public function getData()
    {
        ...
        if($this->request->getParam('crosssellproduct_ids')) {
            $crosssellProductIds = $this->request->getParam('crosssellproduct_ids');
        }
        $productSellerIds = $this->_sellerProduct->getProductCollection()->getAllIds();

        if (!$this->getCollection()->isLoaded()) {
            if (count($crosssellProductIds)) {
                $this->getCollection()->addFieldToFilter('entity_id', array('nin' => $crosssellProductIds))->addFieldToFilter('entity_id', array('in' => $productSellerIds));
            } else {
