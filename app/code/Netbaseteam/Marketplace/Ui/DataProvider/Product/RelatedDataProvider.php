...
class RelatedDataProvider extends ProductDataProvider
{
    ...
    public function getData()
    {
        ...
        if($this->request->getParam('relatedproduct_ids')) {
            $relatedProductIds = $this->request->getParam('relatedproduct_ids');
        }
        $productSellerIds = $this->_sellerProduct->getProductCollection()->getAllIds();

        if (!$this->getCollection()->isLoaded()) {
            if (count($relatedProductIds)) {
                $this->getCollection()->addFieldToFilter('entity_id', array('nin' => $relatedProductIds))->addFieldToFilter('entity_id', array('in' => $productSellerIds));
            } else {
