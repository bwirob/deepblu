...
class MassMarkAsRead extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        if ($isPartner) {
            ...
            foreach ($collection->getItems() as $item) {
                $this->notificationFactory->create()->load($item->getId())->setIsRead(1)->save();
                $notificationRead++;
            }

            if ($notificationRead) {
                $this->messageManager->addSuccessMessage(
                    __('A total of %1 record(s) have been mark as Read.', $notificationRead)
                );
            }
