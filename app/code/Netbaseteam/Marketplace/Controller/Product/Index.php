...
class Index extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        if ($this->customerSession->isLoggedIn()) {
            $productSeller = $this->_product->getProductCollection()->getData();
            $helper = $this->_helper;
            if (($helper->releaseLimit() == 'free' || !$helper->releaseLimit()) && count($productSeller) > 5) {
                $error = "UHJvZHVjdCBNdWx0aXZlbmRvciBpcyBub3QgYWN0aXZl";
                $error ? $this->_messageManager->addError(base64_decode($error)) : '';
            } else {
