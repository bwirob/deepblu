<?php
namespace Netbaseteam\Locator\Ui\Component\Listing\Column;
    ...
    public function prepareDataSource(array $dataSource)
    {
        if(isset($dataSource['data']['items'])) {
            ...
            foreach($dataSource['data']['items'] as & $item) {
                $url = '';
                $localtor = new \Magento\Framework\DataObject($item);
                if($item[$fieldName] != '') {
                    $url = $this->storeManager->getStore()->getBaseUrl(
                            \Magento\Framework\UrlInterface::URL_TYPE_MEDIA
                        ).'locator/store_image/'.$item[$fieldName];
                }
                ...
                $item[$fieldName . '_link'] = $this->urlBuilder->getUrl(
                    'locator/index/edit',
                    ['localtor_id' => $localtor->getLocaltorId(), 'store' => $this->context->getRequestParam('store')]
                );  
