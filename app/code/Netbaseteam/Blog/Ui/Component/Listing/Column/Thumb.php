            ...
            foreach($dataSource['data']['items'] as & $item) {
                ...
                if($item[$fieldName] != '') {
                    if($fieldName=="thumbnail"){
                        ...
                        ).'blog/thumbnail/'.$item[$fieldName];
                    }else{
                        $url = $this->storeManager->getStore()->getBaseUrl(
                            \Magento\Framework\UrlInterface::URL_TYPE_MEDIA
                        ).'blog/image/'.$item[$fieldName];
                    }
                ...
                $item[$fieldName . '_alt'] = $this->getAlt($item) ?: '/blog/post/index';
                $item[$fieldName . '_orig_src'] = $currentUrl;
                $item[$fieldName . '_link'] = $this->urlBuilder->getUrl(
                        'blog/post/edit',
                        ['post_id' => $post->getPostId(), 'store' => $this->context->getRequestParam('store')]
                    );  
